...
public sealed class CreateAppOrderWithIdempotencyCommandValidator
    : AbstractValidator<CreateAppOrderWithIdempotencyCommand>
{
    public CreateAppOrderWithIdempotencyCommandValidator()
    {
...
/// 5. 新订单默认状态为 Addition。
/// </summary>
public sealed class CreateAppOrderWithIdempotencyCommandHandler(
    IRepository<AppOrder> orderRepository,
    IReadRepository<Product> productRepository,
    IUser currentUser)
    : ICommandHandler<CreateAppOrderWithIdempotencyCommand, Result<AppOrderDto>>
{
    public async Task<Result<AppOrderDto>> Handle(
        CreateAppOrderWithIdempotencyCommand request,
